...
- What validation errors looks like
...
TODO: diagram schema -> validateSDL -> GraphQLError[]

```notes
- validateSDL uses a set of validation rules built into graphql-js to validate the service
```
...
TODO: diagram of federated service validation process
...
1. `validateServicesBeforeNormalization`
1. `validateServicesBeforeComposition`
1. `validateSDL` (inside `composeServices`)
1. `validateComposedSchema`
...
- Examples include...
  - Using a reserved field (like `Query._service`)
  - `@external` field on a type definition
...
<pre>
  <h2>validateComposedSchema(schema)</h2>
</pre>
...
- `@requires` / `@provides` / `@key` directive usage
- +6 more
